...
public class Mavenproject8 extends JPanel implements ActionListener {
    ...
    public void cargarSprites() {
        ...
        String[] posiblesRutas = {
            ...
            proyectoDir.getAbsolutePath() + File.separator + "src" + File.separator + "main" + File.separator
            + "java" + File.separator + "com" + File.separator + "mycompany" + File.separator + "sprites",
    ...
    private void cargarSpriteDeArchivo(String nombre, String ruta, int x, int y, int ancho, int alto) {
        try {
            ...
            // Se asume que el archivo PNG contiene los sprites ordenados de una manera
            // específica
            // (ej: fila 1: este, noreste, norte, etc.)
            // Las coordenadas dadas en cargarSprites son aproximadas y deberían ajustarse.
            // Para simplificar, asumiremos que las coordenadas en cargarSprites son
            // correctas para el recorte.
    ...
    private void manejarReglasCruce(Vehiculo vehiculo) {
        ...
        if (enCruce) {
            ...
            if (caminoDespejado(vehiculo)) {
                Vehiculo ahead = findVehicleAhead(vehiculo);
                if (ahead != null && ahead.getVelocidad() == 0) {
                    vehiculo.setVelocidad(0);
                    reglasCruce.incrementarEspera(vehiculo);
                } else {
                    vehiculo.setVelocidad(Vehiculo.getDefaultSpeed());
                }
            } else {
                vehiculo.setVelocidad(0);
                reglasCruce.incrementarEspera(vehiculo);
            ...
            Vehiculo ahead = findVehicleAhead(vehiculo);
            // Distancia de parada segura (DEBE ser mayor a la longitud del vehículo. Ej: 100px (largo) + 20px (espacio))
            final int DISTANCIA_SEGURA = Vehiculo.getDefaultSpeed() * 50; // Aprox. 150px. Si DefaultSpeed=3, 150px.

            if (ahead != null) {
                int distancia = vehiculo.getDistanciaAlFrente(ahead);

                if (distancia <= DISTANCIA_SEGURA) {
                    // Detenerse/Reducir velocidad si estamos muy cerca, *independientemente de la velocidad del carro de adelante*.
                    vehiculo.setVelocidad(0);
                } else if (vehiculo.getVelocidad() == 0 && distancia > DISTANCIA_SEGURA) {
                    // Reanudar si estaba detenido y la distancia es mayor a la segura
                    vehiculo.setVelocidad(Vehiculo.getDefaultSpeed());
                }
                // Nota: Si la distancia es > DISTANCIA_SEGURA y velocidad > 0, el vehículo ya está en velocidad normal.

            } else if (vehiculo.getVelocidad() == 0) {
                // Reanudar si no hay nadie adelante y estaba detenido sin razón de cruce
                vehiculo.setVelocidad(Vehiculo.getDefaultSpeed());
            }
        }
            ...
            for (int j = i + 1; j < vehiculos.size();j++) {
                ...
                if (v1.colisionaCon(v2)) {
                    ...
                    if (v1.getVelocidad() == 0 && v2.getVelocidad() == 0
                            && v1.getDireccion().equals(v2.getDireccion())) {
                        String dir = v1.getDireccion();
                        int gap = 8; 
                        if (dir.equals("este")) {
                            Vehiculo ahead = v1.getX() > v2.getX() ? v1 : v2;
                            Vehiculo behind = ahead == v1 ? v2 : v1;
                            int targetX = ahead.getX() - ahead.getAncho() - gap;
                            if (behind.getX() > targetX) {
                                behind.setX(targetX);
                            }
                            continue;
                            ...
                            Vehiculo ahead = v1.getY() < v2.getY() ? v1 : v2;
                            Vehiculo behind = ahead == v1 ? v2 : v1;
                            int targetY = ahead.getY() + ahead.getAlto() + gap;
                            if (behind.getY() < targetY) {
                                behind.setY(targetY);
                            }
                            continue;
    ...
    private Vehiculo determinarChocador(Vehiculo v1, Vehiculo v2) {
        ...
        // El que está más cerca del centro del otro (y moviéndose hacia él) es el
        // chocador
    ...
    private boolean caminoDespejado(Vehiculo vehiculo) {
        ...
        switch (direccion) {
            case "este":
                areaDelante = new Rectangle(x + vehiculo.getAncho(), y + 10, 60, 60);
                break;
            case "oeste":
            ...
            case "norte":
                ...
                break;
            case "sur":
                areaDelante = new Rectangle(x + 10, y + vehiculo.getAlto(), 60, 60);
                break;
        }
    ...
    public Mavenproject8() {
        ...
        this.reglasCruce = new ReglasCruce(); // Asumiendo que se inicializa aquí
        this.mapaPanel = new MapaPanel();

        // Agregar esta línea:
        this.mapaPanel.setReglasCruce(this.reglasCruce);
        // Timer para animación (60 FPS)
        timer = new Timer(16, this);
        timer.start();

        // Configurar tamaño del panel (ancho expandido para estadísticas)
        setPreferredSize(new Dimension(1200, 720)); // Tamaño ajustado para que todo quepa
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                switch (e.getKeyCode()) {
                    ...
                    case KeyEvent.VK_D ->
                        mostrarDetallesAvanzados = !mostrarDetallesAvanzados;
                    case KeyEvent.VK_T ->
    ...
    private Vehiculo findVehicleAhead(Vehiculo vehiculo) {
        Vehiculo closestAhead = null;
        int minDistance = Integer.MAX_VALUE;
        // Píxeles de tolerancia para considerar el mismo carril (ajustar según la separación)
        final int TOLERANCIA_CARRIL = 15;

        for (Vehiculo otro : vehiculos) {
            // Ignorar el propio vehículo y vehículos con dirección diferente
            if (otro == vehiculo || !otro.getDireccion().equals(vehiculo.getDireccion())) {
                continue;
            }

            // 1. Verificar si están en el mismo carril (mismo X o Y)
            boolean sameLane = false;
            switch (vehiculo.getDireccion()) {
                case "este":
                case "oeste":
                    // Carriles horizontales: verificar Y
                    if (Math.abs(vehiculo.getY() - otro.getY()) < TOLERANCIA_CARRIL) {
                        sameLane = true;
                    }
                    break;
                case "norte":
                case "sur":
                    // Carriles verticales: verificar X
                    if (Math.abs(vehiculo.getX() - otro.getX()) < TOLERANCIA_CARRIL) {
                        sameLane = true;
                    }
                    break;
            }
            ...
            // 2. Calcular la distancia al frente (el vehículo 'otro' debe estar delante de 'vehiculo')
            int distance = vehiculo.getDistanciaAlFrente(otro);

            // SÓLO consideramos un vehículo 'ahead' si la distancia es positiva (está delante)
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Dividir pantalla: 800px para mapa, 400px para estadísticas
        int anchoMapa = 800;
    ...
    private void dibujarPanelEstadisticas(Graphics2D g2d, int inicioX, int ancho) {
        ...
        // Título del panel (fuente más pequeña)
        g2d.setColor(Color.CYAN);
        g2d.setFont(new Font("Arial", Font.BOLD, 18)); // Reducido de 24 a 18
        g2d.drawString("PANEL DE CONTROL", inicioX + 20, 35);

        // Separador
        g2d.setColor(new Color(100, 100, 120));
        g2d.fillRect(inicioX + 20, 45, ancho - 40, 2);

        int yPos = 70;

        // Sección 1: ESTADO ACTUAL (fuentes más pequeñas)
        g2d.setColor(Color.YELLOW);
        g2d.setFont(new Font("Arial", Font.BOLD, 14)); // Reducido de 18 a 14
        g2d.drawString("ESTADO ACTUAL", inicioX + 20, yPos);
        yPos += 25;

        g2d.setColor(Color.WHITE);
        g2d.setFont(new Font("Arial", Font.PLAIN, 12)); // Reducido de 14 a 12
        g2d.drawString("Vehículos activos: " + vehiculos.size(), inicioX + 25, yPos);
        yPos += 22;
        g2d.drawString("Total agregados: " + vehiculosAgregados, inicioX + 25, yPos);
        yPos += 22;
        g2d.drawString("Colisiones activas: " + colisionesActivas.size(), inicioX + 25, yPos);
        yPos += 22;
        g2d.drawString("Reglas: " + (reglasActivadas ? "ACTIVADAS" : "DESACTIVADAS"), inicioX + 25, yPos);
        yPos += 22;
        g2d.drawString("Áreas colisión: " + (mostrarAreasColision ? "VISIBLE" : "OCULTO"), inicioX + 25, yPos);
        ...
        // Barras de distribución (más pequeñas)
        int barWidth = 130; // Reducido de 150
        int barHeight = 16; // Reducido de 20
        int barStartX = inicioX + 25;

        // Este
        g2d.setColor(new Color(0, 150, 255));
        g2d.fillRect(barStartX, yPos, (barWidth * contadoresDir[0]) / Math.max(1, vehiculos.size()), barHeight);
        g2d.setColor(Color.WHITE);
        g2d.drawString("Este: " + contadoresDir[0] + " vehículos", barStartX + 5, yPos + 12);
        yPos += 25;
        ...
        g2d.setColor(Color.WHITE);
        g2d.drawString("Oeste: " + contadoresDir[1] + " vehículos", barStartX + 5, yPos + 12);
        yPos += 25;
        ...
        g2d.setColor(Color.WHITE);
        g2d.drawString("Norte: " + contadoresDir[2] + " vehículos", barStartX + 5, yPos + 12);
        yPos += 25;
        ...
        g2d.setColor(Color.WHITE);
        g2d.drawString("Sur: " + contadoresDir[3] + " vehículos", barStartX + 5, yPos + 12);
        ...
        if (mostrarDetallesAvanzados) {
            ...
            g2d.drawString("Tiempo simulación: " + tiempoTranscurrido + "s", inicioX + 25, yPos);
            yPos += 22;

            g2d.drawString("Total colisiones: " + totalColisiones, inicioX + 25, yPos);
            yPos += 22;

            g2d.drawString("Total giros: " + totalGiros, inicioX + 25, yPos);
            yPos += 22;

            g2d.drawString("Vehículos/hora: " + calcularFlujoPorHora(), inicioX + 25, yPos);
            yPos += 22;

            if (tiempoTranscurrido > 0) {
                g2d.drawString("Colisiones/min: " + String.format("%.2f",
                        (totalColisiones * 60.0) / tiempoTranscurrido), inicioX + 25, yPos);
                yPos += 22;
            }
        }
        ...
        g2d.setFont(new Font("Arial", Font.BOLD, 14)); // Reducido de 16 a 14
        g2d.drawString("CONTROLES", inicioX + 20, yPos);
        yPos += 25;

        g2d.setColor(Color.LIGHT_GRAY);
        g2d.setFont(new Font("Arial", Font.PLAIN, 11)); // Reducido de 12 a 11
        g2d.drawString("A: Agregar 3 vehículos", inicioX + 25, yPos);
        yPos += 18;
        g2d.drawString("C: Limpiar todos", inicioX + 25, yPos);
        yPos += 18;
        g2d.drawString("H: Áreas colisión", inicioX + 25, yPos);
        yPos += 18;
        g2d.drawString("S: Reglas ON/OFF", inicioX + 25, yPos);
        yPos += 18;
        g2d.drawString("SPACE: Resetear reglas", inicioX + 25, yPos);
        yPos += 18;
        g2d.drawString("R: Resetear vehículo", inicioX + 25, yPos);
        yPos += 18;
        g2d.drawString("P: Pausar/Reanudar", inicioX + 25, yPos);
        yPos += 18;
        g2d.drawString("D: Detalles ON/OFF", inicioX + 25, yPos);
        yPos += 18;
        g2d.drawString("E: Exportar datos", inicioX + 25, yPos);

        // Dibujar información del cruce en el lado izquierdo (debajo del mapa)
        dibujarInfoCruce(g2d);
    }

    private void dibujarInfoCruce(Graphics2D g2d) {
        int yPos = 610; // Posición debajo del mapa

        // Fondo para la información del cruce
        g2d.setColor(new Color(20, 20, 30, 200));
        g2d.fillRect(10, yPos, 780, 200);

        // Borde
        g2d.setColor(new Color(60, 60, 80));
        g2d.setStroke(new BasicStroke(2));
        g2d.drawRect(10, yPos, 780, 100);

        // Título
        g2d.setColor(Color.ORANGE);
        g2d.setFont(new Font("Arial", Font.BOLD, 16)); // Fuente ajustada
        g2d.drawString("INFORMACIÓN DEL CRUCE", 20, yPos + 20);

        // Dibujar información específica del cruce
        reglasCruce.dibujarPanelLateral(g2d, 30, yPos + 30);
    }
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("Simulador de Tráfico Inteligente");
        Mavenproject8 simulador = new Mavenproject8();
        frame.add(simulador);

        // Usar tamaño ajustado para que todo quepa
